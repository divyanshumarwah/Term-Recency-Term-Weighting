...
for num, index in enumerate(all_indices):
    ...
    try:
        ...
        while len(resp['hits']['hits']):
            ...
            for res in resp['hits']['hits']:
                if res['_id'] not in indexedIds:
                    ...
                    actions = [
                        dict(_index="trec-news-payloads-index1", _type="_doc", _id=res['_id'], _source=e1[res['_id']])]
            ...
            if len(ids) != 0:
                resultSet = es.mtermvectors(index="trec-news-index", doc_type="news",
                                            body=dict(ids=ids, parameters=dict(term_statistics=True, fields=["text"])))[
                    'docs']
                #procs = []
                '''
                proc = Process(target=getTermWeights)  # instantiating without any argument
                procs.append(proc)
                proc.start()
                '''
                ...
                for doc in resultSet:
                    ...
                    #proc = Process(target=getTermWeights, args=(doc,termWeightsDict))
                    #procs.append(proc)
                    #proc.start()
        ...
        with open('/Users/divyanshumarwah/Documents/dissertation/trec-news-2018/itemsIndexedTest.txt',
                  'a') as filehandle1:
            filehandle1.writelines("%s\n" % itemId for itemId in indexedIds)

        with open('/Users/divyanshumarwah/Documents/dissertation/trec-news-2018/faultyItemsTest.txt',
                  'a') as filehandle2:
